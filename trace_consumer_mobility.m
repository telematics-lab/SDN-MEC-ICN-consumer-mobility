...
sim_list= dir(sprintf('topologies-%d/outsim-*',N)); 
...
voronoi_diagram=ones(10000,10000);
if num_consumers>0
...
for h=1:num_topo
    [c1, c2, c3] = textread(sprintf('topologies-%d/outsim-%d',N, h), '%u %u %u'); % loads the topology generated by BRITE
    ...
    load(sprintf('voronoi_diagrams/voronoi_diagram_for_%d_nodes-%d.mat',N,h),'voronoi_diagram');
    ...
    for w=1:sim_per_topo
        ...
        m_loc_C(1,1)=voronoi_diagram(ceil(m_loc_C(2,1)),ceil(m_loc_C(2,2))); %selects the AP for the generated position from the voronoi mapping
        if num_consumers>0
            for k=1:num_consumers
                loc_C(k,1:2)=round(rand(1,2)*9999)+1;        %generates a random initial position for all other consumers
                AP_C(k)=voronoi_diagram(ceil(loc_C(k,1)),ceil(loc_C(k,2)));    %selects the AP for the generated positions from the voronoi mapping
            end
        end
        loc_P=round(rand(1,2)*9999)+1;         %generates a random initial position for the producer
        AP_P=voronoi_diagram(loc_P(1,1),loc_P(1,2));              %selects the AP for the generated position from the voronoi mapping
        ...
        for s=2:101
            ...
            if m_loc_C(1,1)~=voronoi_diagram(m_loc_C(s,1),m_loc_C(s,2)) %at each step, it checks if the mobile consumer changed AP
                m_loc_C(1,2)=voronoi_diagram(m_loc_C(s,1),m_loc_C(s,2)); %if it does, set the new AP as the the new AP of the mobile consumer
                
                break
            elseif m_loc_C(1,1)==voronoi_diagram(m_loc_C(s,1),m_loc_C(s,2)) && s>100
